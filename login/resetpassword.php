...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Get old password
    $pwdStmt = $conn->prepare("SELECT password FROM userdata WHERE email = ?");
    $pwdStmt->bind_param("s", $email);
    $pwdStmt->execute();
    $pwdStmt->store_result();

    if ($checkEmailStmt->num_rows > 0) {
        $pwdStmt->bind_result($db_password);
        $pwdStmt->fetch();
        ...
        $message = "Email not found. Try creating a new account.";
    }
...
<head>
    ...
    <style>
        ...
        .rst-container input[type="text"],
        .rst-container input[type="password"],
        .rst-container input[type="email"],
        .rst-container input[type="date"]{
            font-family: Verdana;
            width: 100%;
            padding: 10px;
            margin: 8px 0;
